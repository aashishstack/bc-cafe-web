...
function parseHSL(hslStr: string): HslColor {
  ...
  if (!match) return { h: 142, s: 70, l: 45 }; // Metallic Emerald Green
...
function buildGlowVars(glowColor: string, intensity: number): Record<string, string> {
  ...
  for (let i = 0; i < opacities.length; i++) {
    vars[`--glow-color${keys[i]}`] = `hsla(${h}, ${s}%, ${l}%, ${Math.min(opacities[i] * intensity, 1)})`;
  }
  return vars;
}
...
interface BorderGlowProps {
  ...
  glowColor?: string; // HSL numbers space separated e.g. "104 100 54"
...
export function BorderGlow({
  ...
  className = "",
  edgeSensitivity = 10,
  glowColor = "142 70 45", // Metallic Emerald Green
  ...
  coneSpread = 25,
  animated = false,
  colors = ["#004b23", "#00d26a", "#1b4d3e", "#50c878", "#a5c9a5"], // Classic Metallic Greens
  fillOpacity = 0.55,
}: BorderGlowProps) {
